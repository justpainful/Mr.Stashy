...
scheme="${1:-MrStashy}"
destination="${2:-}"
result_bundle="${RESULT_BUNDLE:-Artifacts/MrStashyTests.xcresult}"
log_path="${XCODEBUILD_LOG:-Artifacts/Logs/tests.log}"
...
xcodebuild -project MrStashy.xcodeproj -scheme "$scheme" -configuration Debug \
  -destination "$destination" -only-testing:MrStashyTests \
