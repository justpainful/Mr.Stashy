...
SCREENSHOTS_DIR="$screenshots_dir" \
  ...
  -test-runner-environment SCREENSHOTS_DIR="$screenshots_dir" \
...
find ~/Library/Developer/CoreSimulator/Devices /tmp /var/tmp -path "*/StashyScreenshots/*.png" -exec cp -f {} "$screenshots_dir/" \; 2>/dev/null || true
...
if missing and os.path.exists(result_bundle):
    # Search xcresult for exported PNG attachments matching names
